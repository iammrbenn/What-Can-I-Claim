...
(function () {
  ...
  var Q = [
    ...
    { id: 'district', type: 'choice', label: 'Which council area do you live in?', help: 'We could not look up your postcode automatically.',
      options: R.NOTTS_DISTRICTS.map(function (d) { return [d, d === 'Nottingham' ? 'Nottingham City' : d]; }).concat([['other', 'Somewhere else in England']]),
    ...
    { id: 'water', type: 'choice', label: 'Who supplies your water?', options: [['severn', 'Severn Trent'], ['anglian', 'Anglian Water'], ['other', 'Someone else'], ['unsure', 'Not sure']], show: function (p) { return p.housing && p.housing !== 'family'; } },
  ...
  function viewWelcome() {
    ...
    app.innerHTML =
      ...
      '<p class="muted small">Pilot for Nottinghamshire. National schemes work anywhere in England.</p>' +
      '</section>';
  ...
  function viewResults() {
    ...
    if (!s.inNotts) {
      html += '<p class="notice">' + (profile.country && profile.country !== 'England'
        ? 'You live in ' + esc(profile.country) + '. Many schemes work differently there, so check each result carefully.'
        : 'Local schemes are only included for Nottinghamshire at the moment. National schemes are shown below.') + '</p>';
    }
